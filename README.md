...
location with `LX_INSTALL_DIR`, or pin a version with `LX_VERSION=1.0.5`.
...
```sh
sha256sum -c lx-coreutils-1.0.5-x86_64-unknown-linux-musl.zip.sha256
```
...
```sh
mkdir -p ~/.local/bin
unzip lx-coreutils-1.0.5-x86_64-unknown-linux-musl.zip
mv lx-coreutils-1.0.5-x86_64-unknown-linux-musl/lx* ~/.local/bin/
```
...
Expand-Archive lx-coreutils-1.0.5-x86_64-pc-windows-gnu.zip -DestinationPath .
Copy-Item lx-coreutils-1.0.5-x86_64-pc-windows-gnu\*.exe $dest
```
