...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxdigest"),
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run(&[manifest_dir]);
    out.assert_success();
...
fn json_flag_produces_valid_json() {
    ...
    let manifest_dir = env!("CARGO_MANIFEST_DIR");
    let out = binary().run(&[manifest_dir, "--json"]);
    out.assert_success();
...
fn quiet_flag_suppresses_stderr() {
    ...
    let manifest_dir = env!("CARGO_MANIFEST_DIR");
    let out = binary().run(&[manifest_dir, "--quiet"]);
    out.assert_success();
