...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxcert"),
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run_with_stdin(&[], SAMPLE_PEM);
    out.assert_success();
...
fn json_flag_produces_valid_json_on_stdout() {
    ...
    let out = binary().run_with_stdin(&["--json"], SAMPLE_PEM);
    out.assert_success();
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run_with_stdin(&["--quiet"], SAMPLE_PEM);
    out.assert_success();
