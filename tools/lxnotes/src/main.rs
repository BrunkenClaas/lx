...
fn version_string() -> String {
    lx_core::version::build_version_string("lxnotes", env!("CARGO_PKG_VERSION"))
}

#[derive(Parser)]
#[command(
    name = "lxnotes",
    about = "Structure raw meeting notes into organized sections",
...
struct Cli {
    ...
    /// Disable secret redaction (NOT recommended — secrets in your notes will reach the LLM provider)
    #[arg(long)]
    no_redact: bool,
    ...
    /// Extract action items instead of producing structured sections
    #[arg(long)]
    actions: bool,
}
...
fn main() {
    ...
    // who have audited their notes and accept the risk.
    if cli.no_redact && !cli.quiet {
        eprintln!(
            "warning: --no-redact is set. Secrets in your meeting notes will be sent to \
