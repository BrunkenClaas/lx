...
// Requires: cargo build -p lxnotes before running.
// Tests 1-3 need no network. Tests 4-6 require LX_API_KEY.

use lx_testkit::binary::BinaryUnderTest;

fn binary() -> BinaryUnderTest {
    BinaryUnderTest::for_tool("lxnotes")
}

const SAMPLE_NOTES: &str = include_str!("fixtures/meeting_notes.txt");
...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxnotes"),
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run_with_stdin(&[], SAMPLE_NOTES);
    out.assert_success();
...
fn json_flag_produces_valid_json_on_stdout() {
    ...
    let out = binary().run_with_stdin(&["--json"], SAMPLE_NOTES);
    out.assert_success();
    out.assert_stdout_valid_json();
    out.assert_json_field("sections");
}
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run_with_stdin(&["--quiet"], SAMPLE_NOTES);
    out.assert_success();
