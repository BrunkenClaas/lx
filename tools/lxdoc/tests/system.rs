...
// Requires: cargo build -p lxdoc before running.
// Tests 1-3 need no network. Tests 4-6 require LX_API_KEY.

use lx_testkit::binary::BinaryUnderTest;

fn binary() -> BinaryUnderTest {
    BinaryUnderTest::for_tool("lxdoc")
}

const SAMPLE_CODE: &str = include_str!("fixtures/sample_function.rs");
...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxdoc"),
