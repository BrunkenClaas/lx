...
/// Format: "lxdoc 1.0.0 (lx-coreutils 2026-07, <target>)"
fn version_string() -> String {
    lx_core::version::build_version_string("lxdoc", env!("CARGO_PKG_VERSION"))
}

#[derive(Parser)]
#[command(
    name  = "lxdoc",
    about = "Add docstrings and comments to code",
    // Disable clap's built-in --version; we produce the canonical suite-aware
    // format manually so lxdoc reports the right lx-coreutils label and target.
    disable_version_flag = true
)]
struct Cli {
    /// Code to document (reads from stdin if omitted)
    input: Option<String>,
    ...
    /// Docstring style: auto | docstring | javadoc | rustdoc (default: auto)
    #[arg(long, default_value = "auto", value_name = "STYLE")]
    style: String,
...
fn parse_style(s: &str) -> Result<run::DocStyle, String> {
    match s {
        "auto" => Ok(run::DocStyle::Auto),
        "docstring" => Ok(run::DocStyle::Docstring),
        "javadoc" => Ok(run::DocStyle::Javadoc),
        "rustdoc" => Ok(run::DocStyle::Rustdoc),
        other => Err(format!(
            "unknown style '{other}'; valid values: auto, docstring, javadoc, rustdoc"
        )),
    }
}
...
fn main() {
    ...
    // Parse --style early so we can report bad usage before loading config.
    let style = parse_style(&cli.style).unwrap_or_else(|msg| {
        let e = lx_core::error::LxError::BadUsage(msg);
    ...
    if cli.verbose {
        ...
        eprintln!("[verbose] style: {}", cli.style);
    }
    ...
    let result = if style == run::DocStyle::Auto {
        run::run(&input, &config, client.as_ref())
    } else {
        run::run_with_style(&input, &config, client.as_ref(), &style)
    };
    match result {
        Ok(output) => {
            if cli.json {
