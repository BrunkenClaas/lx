...
fn version_string() -> String {
    lx_core::version::build_version_string("lxmd", env!("CARGO_PKG_VERSION"))
}

#[derive(Parser)]
#[command(
    name = "lxmd",
    about = "Format raw text as clean Markdown",
    disable_version_flag = true
)]
struct Cli {
    /// Raw text to format as Markdown (reads from stdin if omitted)
    input: Option<String>,
