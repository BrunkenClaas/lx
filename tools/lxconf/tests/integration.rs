...
fn oversized_multibyte_input_does_not_panic() {
    ...
    let input = format!("x{}", "ä".repeat(MAX_CONFIG_BYTES_FOR_TEST / 2 + 100));
    let client = MockLlmClient::returning(mock_response_with_findings());
    let config = Config::default();
    let (_out, warnings) = run(&input, None, ConfigMode::Audit, &config, &client).unwrap();
    assert!(
...
/// Mirrors `run::MAX_CONFIG_BYTES`, which is private.
const MAX_CONFIG_BYTES_FOR_TEST: usize = 64000;
