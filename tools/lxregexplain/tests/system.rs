...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxregexplain"),
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run_with_stdin(&[], r"^\d{4}-\d{2}-\d{2}$");
    out.assert_success();
...
fn json_flag_produces_valid_json_on_stdout() {
    ...
    let out = binary().run_with_stdin(&["--json"], r"^\d{4}-\d{2}-\d{2}$");
    out.assert_success();
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run_with_stdin(&["--quiet"], r"^\d{4}-\d{2}-\d{2}$");
    out.assert_success();
