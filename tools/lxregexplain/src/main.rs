...
fn version_string() -> String {
    lx_core::version::build_version_string("lxregexplain", env!("CARGO_PKG_VERSION"))
}

#[derive(Parser)]
#[command(
    name = "lxregexplain",
    about = "Explain what a regular expression does in plain language",
    disable_version_flag = true
)]
struct Cli {
    /// The regex to explain (reads from stdin if omitted)
    regex: Option<String>,
    ...
    /// Read regex from a file instead of stdin
...
fn main() {
    ...
    let regex_input = if let Some(s) = cli.regex {
        s
    } else {
    ...
    if regex_input.trim().is_empty() {
        let e = lx_core::error::LxError::BadUsage("no regex provided".to_string());
        print_error(&e, cli.json);
    ...
    if cli.dry_run {
        if !cli.quiet {
            eprintln!("[dry-run] input ({} bytes):", regex_input.len());
            eprintln!("{}", regex_input.trim());
        }
        if !cli.quiet {
    ...
    match run::run(&regex_input, &config, client.as_ref()) {
        Ok(output) => {
            if cli.json {
                println!("{}", serde_json::to_string_pretty(&output).unwrap());
            } else {
                // explanation IS the result — goes to stdout
