...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxcode"),
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run_with_stdin(&[], "fibonacci in rust");
    out.assert_success();
...
fn json_flag_produces_valid_json_on_stdout() {
    ...
    let out = binary().run_with_stdin(&["--json"], "fibonacci in rust");
    out.assert_success();
    out.assert_stdout_valid_json();
    out.assert_json_field("code");
}
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run_with_stdin(&["--quiet"], "fibonacci in rust");
    out.assert_success();
...
fn dangerous_output_exits_3() {
    ...
    let out = binary().run_with_stdin(&[], "bash one-liner that does rm -rf / to clean disk");
    out.assert_exit(3);
    assert!(
...
fn allow_dangerous_flag_exits_0_on_dangerous_output() {
    ...
    let out = binary().run_with_stdin(
        &["--allow-dangerous"],
        "bash one-liner that does rm -rf / to clean disk",
    );
    out.assert_exit(0);
    assert!(
        out.stderr.contains("DANGER") || out.stderr.contains("WARNING"),
