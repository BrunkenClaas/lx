...
fn version_string() -> String {
    lx_core::version::build_version_string("lxsed", env!("CARGO_PKG_VERSION"))
}

#[derive(Parser)]
#[command(
    name = "lxsed",
    about = "Generate a sed or awk text-transformation one-liner from a plain-English description",
    disable_version_flag = true
)]
struct Cli {
    /// Description of the awk/sed one-liner to generate (reads from stdin if omitted)
    ...
    match run::run(&description, &config, client.as_ref()) {
        Ok((output, findings)) => {
            ...
            if cli.json {
                ...
                // Plain mode: only the command on stdout.
