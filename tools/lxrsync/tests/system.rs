...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxrsync"),
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run(&["copy /home/user/docs to remote backup server"]);
    out.assert_success();
...
fn json_flag_produces_valid_json_on_stdout() {
    ...
    let out = binary().run(&["--json", "mirror /var/www to /mnt/backup"]);
    out.assert_success();
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run(&["--quiet", "sync local docs folder to remote"]);
    out.assert_success();
