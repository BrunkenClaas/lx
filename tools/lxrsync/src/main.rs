...
fn version_string() -> String {
    lx_core::version::build_version_string("lxrsync", env!("CARGO_PKG_VERSION"))
}

#[derive(Parser)]
#[command(
    name  = "lxrsync",
    about = "Generate an rsync command from a plain-English description",
    // Disable clap's built-in --version; we emit the canonical suite format manually.
    disable_version_flag = true
)]
struct Cli {
    /// Plain-English description of the desired rsync operation
        ...
        Ok((output, findings)) => {
            ...
            if cli.json {
                ...
                if lx_core::output::show_narration(cli.quiet, cli.verbose) {
                    eprintln!("# rsync command generated");
                }
            }
