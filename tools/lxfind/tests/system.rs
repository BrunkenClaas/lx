...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxfind"),
...
fn stdout_is_pipe_safe() {
    ...
    // Use current directory and a realistic description.
    let out = binary().run(&["configuration file for the project", "."]);
    out.assert_success();
...
fn json_flag_produces_valid_json() {
    ...
    let out = binary().run(&["--json", "configuration file for the project", "."]);
    out.assert_success();
    out.assert_stdout_valid_json();
    out.assert_json_field("paths");
}
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run(&["--quiet", "configuration file for the project", "."]);
    out.assert_success();
