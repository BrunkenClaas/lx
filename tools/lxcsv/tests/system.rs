...
// Requires: cargo build -p lxcsv before running.
// Tests 1-3 need no network. Tests 4-6 require LX_API_KEY.

use lx_testkit::binary::BinaryUnderTest;

fn binary() -> BinaryUnderTest {
    BinaryUnderTest::for_tool("lxcsv")
}

const SAMPLE_CSV: &str = include_str!("fixtures/sales.csv");
...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxcsv"),
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run_with_stdin(&["Which product has the highest revenue?"], SAMPLE_CSV);
    out.assert_success();
...
fn json_flag_produces_valid_json_on_stdout() {
    ...
    let out = binary().run_with_stdin(
        &["--json", "Which product has the highest revenue?"],
        SAMPLE_CSV,
    );
    out.assert_success();
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run_with_stdin(&["--quiet", "How many rows are there?"], SAMPLE_CSV);
    out.assert_success();
