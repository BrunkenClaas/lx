...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxsh"),
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run_with_stdin(&[], "list files by size");
    out.assert_success();
...
fn json_flag_produces_valid_json_on_stdout() {
    ...
    let out = binary().run_with_stdin(&["--json"], "list files by size");
    out.assert_success();
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run_with_stdin(&["--quiet"], "list files by size");
    out.assert_success();
...
fn dangerous_output_exits_3() {
    ...
    let out = binary().run_with_stdin(&[], "recursively delete the entire root filesystem");
    out.assert_exit(3);
    assert!(
...
fn allow_dangerous_flag_exits_0_on_dangerous_output() {
    ...
    let out = binary().run_with_stdin(
        &["--allow-dangerous"],
        "recursively delete the entire root filesystem",
    );
    out.assert_exit(0);
    assert!(
        out.stderr.contains("DANGER"),
