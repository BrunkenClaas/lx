...
fn binary() -> BinaryUnderTest {
    BinaryUnderTest::for_tool("lxfixcmd")
}
...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(out.stdout.starts_with("lxfixcmd"));
}
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run(&["git psh origin main"]);
    out.assert_success();
...
fn json_flag_produces_valid_json() {
    ...
    let out = binary().run(&["--json", "git psh origin main"]);
    out.assert_success();
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run(&["--quiet", "git psh origin main"]);
    out.assert_success();
}
