...
// Requires: cargo build -p lxstandup before running.
// Tests 1-3 need no network. Tests 4-6 require LX_API_KEY.

use lx_testkit::binary::BinaryUnderTest;

fn binary() -> BinaryUnderTest {
    BinaryUnderTest::for_tool("lxstandup")
}

const SAMPLE_INPUT: &str = include_str!("fixtures/git_log.txt");
...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxstandup"),
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run_with_stdin(&[], SAMPLE_INPUT);
    out.assert_success();
...
fn json_flag_produces_valid_json() {
    ...
    let out = binary().run_with_stdin(&["--json"], SAMPLE_INPUT);
    out.assert_success();
    out.assert_stdout_valid_json();
    out.assert_json_field("done");
}
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run_with_stdin(&["--quiet"], SAMPLE_INPUT);
    out.assert_success();
