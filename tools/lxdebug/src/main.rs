...
fn version_string() -> String {
    lx_core::version::build_version_string("lxdebug", env!("CARGO_PKG_VERSION"))
}

#[derive(Parser)]
#[command(
    name = "lxdebug",
    about = "Analyse an error message and suggest a fix",
...
struct Cli {
    ...
    /// Disable secret redaction (NOT recommended — secrets in error output will reach the LLM provider)
    #[arg(long)]
    no_redact: bool,
...
fn main() {
    ...
    if cli.no_redact && !cli.quiet {
        eprintln!(
            "warning: --no-redact is set. Secrets in your error output will be sent to \
