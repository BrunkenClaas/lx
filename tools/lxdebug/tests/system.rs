...
// Requires: cargo build -p lxdebug before running.
// Tests 1-3 need no network. Tests 4-6 require LX_API_KEY.

use lx_testkit::binary::BinaryUnderTest;

fn binary() -> BinaryUnderTest {
    BinaryUnderTest::for_tool("lxdebug")
}

const ERROR_LOG: &str = include_str!("fixtures/enoent_error.txt");
...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxdebug"),
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run_with_stdin(&[], ERROR_LOG);
    out.assert_success();
...
fn json_flag_produces_valid_json_on_stdout() {
    ...
    let out = binary().run_with_stdin(&["--json"], ERROR_LOG);
    out.assert_success();
    out.assert_stdout_valid_json();
    out.assert_json_field("fix");
}
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run_with_stdin(&["--quiet"], ERROR_LOG);
    out.assert_success();
