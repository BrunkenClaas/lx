...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxsecret"),
...
fn json_flag_produces_valid_json_on_stdout() {
    ...
    let out = binary().run_with_stdin(&["--json"], "AWS_ACCESS_KEY_ID=AKIAIOSFODNN7EXAMPLE");
    out.assert_success();
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run_with_stdin(&["--quiet"], "AWS_ACCESS_KEY_ID=AKIAIOSFODNN7EXAMPLE");
    out.assert_success();
    ...
    assert!(
        ...
        "--quiet must suppress stderr, got: {stderr_lines:?}"
    );
}
