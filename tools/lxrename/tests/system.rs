...
fn binary() -> BinaryUnderTest {
    BinaryUnderTest::for_tool("lxrename")
}
...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(out.stdout.starts_with("lxrename"));
}
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run_with_stdin(&["rename to snake_case"], "testFoo.py\ntestBar.py\n");
    out.assert_success();
...
fn json_flag_produces_valid_json() {
    ...
    let out = binary().run_with_stdin(&["--json", "rename to snake_case"], "testFoo.py\n");
    out.assert_success();
    out.assert_stdout_valid_json();
    out.assert_json_field("renames");
}
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run_with_stdin(&["--quiet", "rename to snake_case"], "testFoo.py\n");
    out.assert_success();
}
