...
// Requires: cargo build -p lxproof before running.
// Tests 1-3 need no network. Tests 4-6 require LX_API_KEY.

use lx_testkit::binary::BinaryUnderTest;

fn binary() -> BinaryUnderTest {
    BinaryUnderTest::for_tool("lxproof")
}

// A realistic proofreading input for integration tests.
const PROOF_INPUT: &str = "I recieved you're letter yesturday and was very gratefull for it.";
...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxproof"),
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run_with_stdin(&[], PROOF_INPUT);
    out.assert_success();
...
fn json_flag_produces_valid_json_on_stdout() {
    ...
    let out = binary().run_with_stdin(&["--json"], PROOF_INPUT);
    out.assert_success();
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run_with_stdin(&["--quiet"], PROOF_INPUT);
    out.assert_success();
