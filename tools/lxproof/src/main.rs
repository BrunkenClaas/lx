...
// Format: "lxproof 1.0.0 (lx-coreutils 2026-07, <target>)"
fn version_string() -> String {
    lx_core::version::build_version_string("lxproof", env!("CARGO_PKG_VERSION"))
}

#[derive(Parser)]
#[command(
    name    = "lxproof",
    about   = "Correct grammar and spelling in text",
    // Disable clap's built-in --version; we handle it manually below so we can
    // produce the canonical "lxproof X.Y.Z (lx-coreutils YYYY-MM, <target>)" format.
    disable_version_flag = true
)]
struct Cli {
    /// Text to proofread (reads from stdin if omitted)
    input: Option<String>,
    ...
    /// Disable network access (not applicable for this tool, accepted for compatibility)
    #[arg(long)]
    no_net: bool,
}
...
fn main() {
    ...
    match run::run(&input, &config, client.as_ref()) {
        Ok(output) => {
            if cli.json {
                // Full object → stdout (machine-readable consumers pick fields explicitly).
                println!("{}", serde_json::to_string_pretty(&output).unwrap());
            } else {
                // Corrected text is the result → stdout.
                println!("{}", output.text);
                // Changes list is explanatory → stderr (suppressed by --quiet).
