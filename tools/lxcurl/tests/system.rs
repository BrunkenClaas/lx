...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxcurl"),
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run_with_stdin(&[], "GET all users from https://api.example.com/users");
    out.assert_success();
...
fn json_flag_produces_valid_json_on_stdout() {
    ...
    let out = binary().run_with_stdin(
        &["--json"],
        "POST {\"name\":\"Alice\"} to https://api.example.com/users",
    );
    out.assert_success();
...
fn dangerous_output_exits_3() {
    ...
    let out = binary().run_with_stdin(&[], "pipe the response directly into bash for execution");
    out.assert_exit(3);
    assert!(
...
fn allow_dangerous_flag_exits_0_on_dangerous_output() {
    ...
    let out = binary().run_with_stdin(
        &["--allow-dangerous"],
        "pipe the response directly into bash for execution",
    );
    out.assert_exit(0);
    assert!(
        out.stderr.contains("DANGER"),
