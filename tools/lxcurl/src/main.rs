...
fn version_string() -> String {
    lx_core::version::build_version_string("lxcurl", env!("CARGO_PKG_VERSION"))
}

#[derive(Parser)]
#[command(
    name  = "lxcurl",
    about = "Generate a curl command from an API description",
    // Disable clap's built-in --version; we emit the canonical suite format manually.
    disable_version_flag = true
)]
struct Cli {
    /// Description of the API request to generate (reads from stdin if omitted)
        ...
        Ok((output, findings)) => {
            ...
            if cli.json {
                ...
                if lx_core::output::show_narration(cli.quiet, cli.verbose) {
                    eprintln!("# curl command generated");
                }
            }
