...
// Requires: cargo build -p lxpull before running.
// Tests 1-3 need no network. Tests 4-6 require LX_API_KEY.

use lx_testkit::binary::BinaryUnderTest;

fn binary() -> BinaryUnderTest {
    BinaryUnderTest::for_tool("lxpull")
}

const SAMPLE_CONTACTS: &str = include_str!("fixtures/contacts.txt");
...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxpull"),
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run_with_stdin(&["--fields", "name,email"], SAMPLE_CONTACTS);
    out.assert_success();
...
fn json_flag_produces_valid_json_on_stdout() {
    ...
    let out = binary().run_with_stdin(&["--json", "--fields", "name,email"], SAMPLE_CONTACTS);
    out.assert_success();
    out.assert_stdout_valid_json();
    out.assert_json_field("records");
}
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run_with_stdin(&["--quiet", "--fields", "name,email"], SAMPLE_CONTACTS);
    out.assert_success();
