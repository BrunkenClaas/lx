...
fn version_string() -> String {
    lx_core::version::build_version_string("lxtypehint", env!("CARGO_PKG_VERSION"))
}

#[derive(Parser)]
#[command(
    name  = "lxtypehint",
    about = "Add type hints and annotations to source code",
    // Disable clap's built-in --version; we emit the canonical suite format manually.
    disable_version_flag = true
)]
struct Cli {
    /// Source code to annotate (reads from stdin if omitted)
    input: Option<String>,
    ...
    /// Show input that would be sent to the LLM, then exit
    #[arg(long)]
    dry_run: bool,
...
fn main() {
    ...
    if input.trim().is_empty() {
        let e = lx_core::error::LxError::BadUsage("no code provided".to_string());
        print_error(&e, cli.json);
        process::exit(exit::BAD_USAGE);
    }

    // --dry-run: show input that would be sent, then exit.
    if cli.dry_run {
        run::warn_findings(&run::check(input.trim()));
        if !cli.quiet {
            eprintln!(
    ...
    match run::run(&input, &config, client.as_ref()) {
        Ok((output, findings)) => {
            ...
            // The annotated code goes to stdout as text only — never executed.
            run::warn_findings(&findings);
            if cli.json {
                println!("{}", serde_json::to_string_pretty(&output).unwrap());
            } else {
                // The annotated code IS the result — print it to stdout (pipe-safe).
                println!("{}", output.code);
            }
            process::exit(exit::SUCCESS);
        }
