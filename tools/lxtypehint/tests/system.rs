...
// Requires: cargo build -p lxtypehint before running.
// Tests 1-3 need no network. Tests 4-6 require LX_API_KEY.

use lx_testkit::binary::BinaryUnderTest;

fn binary() -> BinaryUnderTest {
    BinaryUnderTest::for_tool("lxtypehint")
}

const SAMPLE_CODE: &str = include_str!("fixtures/basic.py");
...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxtypehint"),
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run_with_stdin(&[], SAMPLE_CODE);
    out.assert_success();
...
fn json_flag_produces_valid_json() {
    ...
    let out = binary().run_with_stdin(&["--json"], SAMPLE_CODE);
    out.assert_success();
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run_with_stdin(&["--quiet"], SAMPLE_CODE);
    out.assert_success();
