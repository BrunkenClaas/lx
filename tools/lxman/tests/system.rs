...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxman"),
...
fn json_flag_produces_valid_json_on_stdout() {
    ...
    let out = binary().run(&["--json", "git-stash"]);
    out.assert_success();
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run(&["--quiet", "git-stash"]);
    out.assert_success();
