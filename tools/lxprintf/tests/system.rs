...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxprintf"),
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run_with_stdin(&[], "ISO date");
    out.assert_success();
...
fn json_flag_produces_valid_json() {
    ...
    let out = binary().run_with_stdin(&["--json"], "ISO date");
    out.assert_success();
    out.assert_stdout_valid_json();
    out.assert_json_field("format");
}
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run_with_stdin(&["--quiet"], "ISO date");
    out.assert_success();
