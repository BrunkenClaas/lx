...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxdockercmd"),
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run(&["run nginx on port 8080"]);
    out.assert_success();
...
fn json_flag_produces_valid_json_on_stdout() {
    ...
    let out = binary().run(&["--json", "list all running containers"]);
    out.assert_success();
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run(&["--quiet", "show logs for container named web"]);
    out.assert_success();
...
fn dangerous_output_exits_3() {
    ...
    let out = binary().run(&["remove all containers and volumes with force"]);
    out.assert_exit(3);
    assert!(
...
fn allow_dangerous_flag_exits_0_on_dangerous_output() {
    ...
    let out = binary().run(&[
        "--allow-dangerous",
        "remove all containers and volumes with force",
    ]);
    out.assert_exit(0);
    assert!(
        out.stderr.contains("DANGER"),
