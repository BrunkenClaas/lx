...
fn version_string() -> String {
    lx_core::version::build_version_string("lxdockercmd", env!("CARGO_PKG_VERSION"))
}

#[derive(Parser)]
#[command(
    name  = "lxdockercmd",
    about = "Generate a docker command from a plain-English description",
    // Disable clap's built-in --version; we emit the canonical suite format manually.
    disable_version_flag = true
)]
struct Cli {
    /// The description of the docker command to generate (e.g. "run nginx on port 8080")
        ...
        Ok((output, findings)) => {
            ...
            if cli.json {
                ...
                if lx_core::output::show_narration(cli.quiet, cli.verbose) {
                    eprintln!("# docker command generated");
                }
            }
