...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxmakefile"),
...
fn create_mode_stdout_is_pipe_safe() {
    ...
    let out = binary().run_with_stdin(&[], "build, test, and clean a Rust project");
    out.assert_success();
...
fn json_flag_produces_valid_json() {
    ...
    let out = binary().run_with_stdin(&["--json"], "build, test, and clean a Rust project");
    out.assert_success();
...
fn edit_mode_produces_output() {
    ...
    let out = binary().run_with_stdin(&["add a test target"], existing);
    out.assert_success();
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run_with_stdin(&["--quiet"], "build, test, and clean a Rust project");
    out.assert_success();
