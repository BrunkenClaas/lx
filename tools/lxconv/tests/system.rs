...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxconv"),
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run_with_stdin(&["json"], "city,pop\nBerlin,3700000\nParis,2100000\n");
    out.assert_success();
