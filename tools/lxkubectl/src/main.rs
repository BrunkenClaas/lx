...
fn version_string() -> String {
    lx_core::version::build_version_string("lxkubectl", env!("CARGO_PKG_VERSION"))
}

#[derive(Parser)]
#[command(
    name  = "lxkubectl",
    about = "Generate a kubectl command from a plain-English description",
    // Disable clap's built-in --version; we emit the canonical suite format manually.
    disable_version_flag = true
)]
struct Cli {
    /// The description to translate into a kubectl command (e.g. "list all pods in production")
        ...
        Ok((output, findings)) => {
            ...
            if cli.json {
                ...
                if lx_core::output::show_narration(cli.quiet, cli.verbose) {
                    eprintln!("# kubectl command generated");
                }
            }
            // DANGER warning already printed in run(); exit 3 unless overridden.
