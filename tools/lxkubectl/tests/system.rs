...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxkubectl"),
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run(&["list all pods in the default namespace"]);
    out.assert_success();
...
fn json_flag_produces_valid_json_on_stdout() {
    ...
    let out = binary().run(&["--json", "get all deployments in the kube-system namespace"]);
    out.assert_success();
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run(&["--quiet", "show logs for pod web-123"]);
    out.assert_success();
