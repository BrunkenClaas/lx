...
// Requires: cargo build -p lxurl before running.
// Tests 1-3 need no network. Tests 4-6 require LX_API_KEY + network access.

use lx_testkit::binary::BinaryUnderTest;

fn binary() -> BinaryUnderTest {
    BinaryUnderTest::for_tool("lxurl")
}
...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxurl"),
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run(&["https://example.com"]);
    out.assert_success();
...
fn json_flag_produces_valid_json_on_stdout() {
    ...
    let out = binary().run(&["--json", "https://example.com"]);
    out.assert_success();
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run(&["--quiet", "https://example.com"]);
    out.assert_success();
