...
fn version_string() -> String {
    lx_core::version::build_version_string("lxpr", env!("CARGO_PKG_VERSION"))
}

#[derive(Parser)]
#[command(
    name    = "lxpr",
    about   = "Generate a pull-request description from a diff or commit log (pipe git diff | git log -p)",
...
fn main() {
    ...
    // --dry-run: redact first, then show what *would* be sent, then exit.
    if cli.dry_run {
    ...
    match result {
        Ok((output, warnings)) => {
            ...
            if cli.json {
                ...
                // The full PR text (title + body) IS the result — it gets piped/redirected.
