...
fn version_string() -> String {
    lx_core::version::build_version_string("lxmermaid", env!("CARGO_PKG_VERSION"))
}

#[derive(Parser)]
#[command(
    name = "lxmermaid",
    about = "Generate a Mermaid diagram from a plain-language description",
    // Disable clap's built-in --version; we emit the canonical suite format manually.
    disable_version_flag = true
)]
struct Cli {
    /// Description of the diagram to generate (reads from stdin if omitted)
    description: Option<String>,

    /// Output as JSON (includes all fields)
    #[arg(long)]
    json: bool,
...
fn main() {
    ...
    if cli.dry_run {
        if !cli.quiet {
            ...
            if let Some(ref ex) = existing {
                eprintln!("[dry-run] existing diagram ({} bytes):", ex.trim().len());
                eprintln!("{}", ex.trim());
            }
    ...
    match run::run(&description, existing.as_deref(), &config, client.as_ref()) {
        Ok((output, findings)) => {
            ...
            if cli.json {
                ...
                // Plain mode: only the diagram on stdout.
