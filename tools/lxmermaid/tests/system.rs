...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxmermaid"),
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run_with_stdin(&[], "simple flowchart with start and end nodes");
    out.assert_success();
...
fn json_flag_produces_valid_json_on_stdout() {
    ...
    let out = binary().run_with_stdin(&["--json"], "simple flowchart with start and end nodes");
    out.assert_success();
    out.assert_stdout_valid_json();
    out.assert_json_field("diagram");
}
...
fn edit_mode_produces_output() {
    ...
    let existing_diagram = "flowchart TD\n    A[Start] --> B[End]";
    let out = binary().run_with_stdin(&["add a middle step C between A and B"], existing_diagram);
    out.assert_success();
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run_with_stdin(&["--quiet"], "simple flowchart with start and end nodes");
    out.assert_success();
