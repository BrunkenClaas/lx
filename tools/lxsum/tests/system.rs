...
// Requires: cargo build -p lxsum before running.
// Tests 1-3 need no network. Tests 4-6 require LX_API_KEY.

use lx_testkit::binary::BinaryUnderTest;

fn binary() -> BinaryUnderTest {
    BinaryUnderTest::for_tool("lxsum")
}

const LOG_FIXTURE: &str = include_str!("fixtures/deployment_log.txt");
...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxsum"),
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run_with_stdin(&[], LOG_FIXTURE);
    out.assert_success();
...
fn json_flag_produces_valid_json_on_stdout() {
    ...
    let out = binary().run_with_stdin(&["--json"], LOG_FIXTURE);
    out.assert_success();
    out.assert_stdout_valid_json();
    out.assert_json_field("tldr");
}
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run_with_stdin(&["--quiet"], LOG_FIXTURE);
    out.assert_success();
