...
fn version_string() -> String {
    lx_core::version::build_version_string("lxcommit", env!("CARGO_PKG_VERSION"))
}

#[derive(Parser)]
#[command(
    name    = "lxcommit",
    about   = "Generate a Conventional Commit message from a git diff (pipe git diff --staged)",
...
fn main() {
    ...
    // run() always redacts internally. Pass no_redact flag through config-level
    // by passing the raw diff — run() reads config.redact.level and applies redaction.
    // When --no-redact is set we still call run() but skip the redact step inside.
