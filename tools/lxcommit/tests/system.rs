...
// Requires: cargo build -p lxcommit before running.
// Tests 1-3 need no network. Tests 4-6 require LX_API_KEY.

use lx_testkit::binary::BinaryUnderTest;

fn binary() -> BinaryUnderTest {
    BinaryUnderTest::for_tool("lxcommit")
}

const SAMPLE_DIFF: &str = include_str!("fixtures/sample_add_function.diff");
...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxcommit"),
