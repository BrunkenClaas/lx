...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxgitignore"),
...
fn edit_mode_produces_output() {
    ...
    let out = binary().run_with_stdin(&["also ignore .env files"], existing);
    out.assert_success();
