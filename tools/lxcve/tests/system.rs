...
fn binary() -> BinaryUnderTest {
    BinaryUnderTest::for_tool("lxcve")
}

const SAMPLE_LOCKFILE: &str = include_str!("fixtures/cargo_lock_vulnerable.toml");
...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxcve"),
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run_with_stdin(&[], SAMPLE_LOCKFILE);
    out.assert_success();
...
fn json_flag_produces_valid_json_on_stdout() {
    ...
    let out = binary().run_with_stdin(&["--json"], SAMPLE_LOCKFILE);
    out.assert_success();
    out.assert_stdout_valid_json();
    out.assert_json_field("vulns");
}
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run_with_stdin(&["--quiet"], SAMPLE_LOCKFILE);
    out.assert_success();
