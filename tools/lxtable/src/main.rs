...
fn version_string() -> String {
    lx_core::version::build_version_string("lxtable", env!("CARGO_PKG_VERSION"))
}

#[derive(Parser)]
#[command(
    name = "lxtable",
    about = "Convert unstructured text into a Markdown table",
    disable_version_flag = true
)]
struct Cli {
    /// Text to convert (reads from stdin if omitted)
    input: Option<String>,
    ...
    match run::run(&input, &config, client.as_ref()) {
        Ok(output) => {
            if cli.json {
                ...
                // Markdown table → stdout (the table IS the result)
