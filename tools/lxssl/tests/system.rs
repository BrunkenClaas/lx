...
fn binary() -> BinaryUnderTest {
    BinaryUnderTest::for_tool("lxssl")
}
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run_with_stdin(&[], "verify error:num=10:certificate has expired");
    out.assert_success();
...
fn json_flag_produces_valid_json() {
    ...
    let out = binary().run_with_stdin(&["--json"], "verify error:num=10:certificate has expired");
    out.assert_success();
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run_with_stdin(&["--quiet"], "verify error:num=10:certificate has expired");
    out.assert_success();
}
