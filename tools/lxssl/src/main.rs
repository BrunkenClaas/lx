...
fn version_string() -> String {
    lx_core::version::build_version_string("lxssl", env!("CARGO_PKG_VERSION"))
}

#[derive(Parser)]
#[command(
    name = "lxssl",
    about = "Diagnose TLS/certificate errors from openssl or curl output",
    disable_version_flag = true
)]
struct Cli {
    /// Optional hostname the TLS output relates to
    host: Option<String>,
...
fn main() {
    ...
    // stdin is REQUIRED when no --file provided and no TTY input
    if cli.file.is_none() && platform::is_tty(Fd::Stdin) {
        let e = lx_core::error::LxError::BadUsage(
            "no input: pipe TLS tool output or use --file".to_string(),
        );
        print_error(&e, cli.json);
    ...
    let host = cli.host.as_deref().unwrap_or("");
    ...
    match run::run(&input, host, &config, client.as_ref()) {
        Ok(output) => {
            if cli.json {
