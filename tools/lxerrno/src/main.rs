...
fn version_string() -> String {
    lx_core::version::build_version_string("lxerrno", env!("CARGO_PKG_VERSION"))
}

#[derive(Parser)]
#[command(
    name    = "lxerrno",
    about   = "Explain an error code: HTTP status, POSIX errno, or exit code",
    // Disable clap's built-in --version; handled manually for canonical suite format.
    disable_version_flag = true
)]
struct Cli {
    /// The error code to explain (reads from stdin if omitted).
    /// Examples: 404, ENOENT, 28, "exit 130", "errno 12"
    input: Option<String>,
    ...
    /// Show what would be sent to the LLM without sending it
    #[arg(long)]
    dry_run: bool,
...
fn main() {
    ...
    // --version: canonical suite-aware format.
    if cli.version {
    ...
    // Collect input: argument > --file > stdin.
    ...
    if input.trim().is_empty() {
        let e = lx_core::error::LxError::BadUsage("no error code provided".to_string());
        print_error(&e, cli.json);
    ...
    if cli.dry_run {
        if !cli.quiet {
            eprintln!("[dry-run] input ({} bytes): {}", input.len(), input.trim());
        }
        if !cli.quiet {
    ...
    match run(&input, &config, client.as_ref()) {
        Ok(output) => {
            if cli.json {
