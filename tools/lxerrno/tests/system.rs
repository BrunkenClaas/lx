...
fn version_flag_exits_0_and_format_correct() {
    ...
    assert!(
        out.stdout.trim().starts_with("lxerrno"),
...
fn stdout_is_pipe_safe() {
    ...
    let out = binary().run_with_stdin(&[], "ENOENT");
    out.assert_success();
...
fn json_flag_produces_valid_json_on_stdout() {
    ...
    let out = binary().run_with_stdin(&["--json"], "ENOENT");
    out.assert_success();
    out.assert_stdout_valid_json();
    out.assert_json_field("meaning");
}
...
fn quiet_flag_suppresses_stderr() {
    ...
    let out = binary().run_with_stdin(&["--quiet"], "ENOENT");
    out.assert_success();
